...
def gelu_forward_triton(x: torch.Tensor) -> torch.Tensor:
    """Compute GELU(x) with tanh approximation on CUDA float32 or bfloat16.

    Arithmetic is always done in float32; bfloat16 inputs are upcast on load
    ...
    _gelu_kernel[grid](x, out, n, BLOCK=BLOCK, DTYPE=_TRITON_DTYPE[x.dtype])
    return out
