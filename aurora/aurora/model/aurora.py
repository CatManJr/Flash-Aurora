        ...
        *,
        ...
        Args:
            ...
            compile_backbone (bool, optional): If ``True``, wrap :class:`Swin3DTransformerBackbone` with
                ...
                up. Requires PyTorch 2.0+. Defaults to ``False``.
            compile_backbone_mode (str, optional): ``mode`` passed to :func:`torch.compile`, e.g.
                ``"default"``, ``"reduce-overhead"``, or ``"max-autotune"``. If ``None``, the
                compiler default is used.
            compile_backbone_dynamic (bool, optional): ``dynamic`` passed to :func:`torch.compile`.
                ``True`` (default) supports varying ``patch_res`` / batch; on some setups Inductor
                may log ``sympy`` warnings or compile slowly. Set ``False`` if inputs are always the
                same shape (e.g. fixed-resolution deployment) to avoid that class of issues.
            workspace_pool (InferenceWorkspacePool, optional): Optional :class:`InferenceWorkspacePool`
                passed to the Swin backbone to reuse scratch buffers (e.g. last decoder concat) on
        ...
        self.compile_backbone = compile_backbone
        self.compile_backbone_dynamic = compile_backbone_dynamic
        if compile_backbone:
            compile_kw: dict[str, Any] = {"dynamic": compile_backbone_dynamic}
            if compile_backbone_mode is not None:
                compile_kw["mode"] = compile_backbone_mode
            self.backbone = torch.compile(self.backbone, **compile_kw)
