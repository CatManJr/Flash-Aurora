...
_MISSING_FOUNDRY_DEPS: list[str] = []
try:
    import mlflow  # noqa: F401
    from aurora.foundry.server.mlflow_wrapper import AuroraModelWrapper
except ModuleNotFoundError:
    AuroraModelWrapper = None  # type: ignore[assignment]
    _MISSING_FOUNDRY_DEPS.append("mlflow")

try:
    from azure.storage.blob import BlobClient
except ModuleNotFoundError:
    BlobClient = None
    _MISSING_FOUNDRY_DEPS.append("azure-storage-blob")
...
class _MockContext:
    ...
    def __init__(self, checkpoint_path: str) -> None:
        self.artifacts = {"aurora-0.25-small-pretrained": checkpoint_path}


def _server_work(
    queue_in: Queue[dict | None],
    ...
    """
    try:
        context = _MockContext(checkpoint_path)
...
def mock_foundry_client(
    ...
    if _MISSING_FOUNDRY_DEPS:
        pytest.skip(
            "Foundry tests require optional dependencies: " + ", ".join(_MISSING_FOUNDRY_DEPS)
        )
    if not request.config.pluginmanager.hasplugin("requests_mock"):
        pytest.skip("Foundry tests require pytest-requests-mock plugin.")
    ...
    try:
        ...
        pytest.skip(f"Foundry tests require Hub access for checkpoint download: {exc}")
    ...
    th = Thread(target=_server_work, args=(queue_in, queue_out, checkpoint_path))
    th.start()
