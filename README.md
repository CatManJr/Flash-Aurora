...
- **Triton ops** (CUDA inference) under [`aurora/aurora/ops/`](aurora/aurora/ops/): window **layout**, **AdaLN**, **Perceiver LN**. MLP uses PyTorch/cuBLAS; planned fusion in [`cute/mlp_ffn.py`](aurora/aurora/ops/cute/mlp_ffn.py) (CuTe / [cuDNN frontend](https://github.com/NVIDIA/cudnn-frontend/tree/develop/python/cudnn)). Legacy tanh GELU: `triton_gelu.py` via `use_triton_mlp=True`.
