...
- **Triton ops** (CUDA float32 inference paths) under [`aurora/aurora/ops/`](aurora/aurora/ops/): window **layout** in [`triton_swin3d_layout.py`](aurora/aurora/ops/triton_swin3d_layout.py), **AdaLN** in `triton_adaln.py`. Inference presets use PyTorch `nn.GELU` (optional legacy `triton_gelu.py` via `use_triton_mlp=True`).
