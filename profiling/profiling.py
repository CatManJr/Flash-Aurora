...
# Repo root on sys.path for ``uv run python profiling/profiling.py`` without editable install.
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Capture a fixed-shape CUDA graph after warmup. Requires fast_fp32 "
    ...
    if args.cuda_graph:
        ...
        if args.inference_precision in (None, "fp32"):
            raise SystemExit(
                "--cuda-graph requires --inference-precision tf32, bf16_mixed, or bf16."
            )
