...
def matching_small_batch() -> Batch:
    height, width = 400, 800
    levels = (50, 250, 500, 600, 700, 850, 925)
    lat = torch.linspace(90, -90, height)
    lon = torch.arange(0, 360, 360 / width)
    return Batch(
        surf_vars={k: torch.randn(1, 2, height, width) for k in ("2t", "10u", "10v", "msl")},
        static_vars={k: torch.randn(height, width) for k in ("lsm", "slt", "z")},
        atmos_vars={k: torch.randn(1, 2, len(levels), height, width) for k in ("u", "v", "t", "q")},
        metadata=Metadata(
            lat=lat,
            lon=lon,
            time=(datetime(2020, 6, 1, 12, 0),),
