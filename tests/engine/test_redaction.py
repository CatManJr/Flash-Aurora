...
from flash_aurora.engine.core.redaction import (
    redact_text,
    safe_config_label,
    safe_path,
    sanitize_exception,
...
def test_safe_path_hides_home_prefix(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("flash_aurora.engine.core.redaction.Path.home", lambda: tmp_path)
...
def test_cds_config_error_uses_safe_label(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    from flash_aurora.engine.ingress.download.cds import CdsConfigError, cds_client
