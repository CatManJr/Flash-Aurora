...
from engine.ingress.download.downloader import DataDownloader, DownloadRequest
from engine.ingress.download.layout import expected_paths, missing_keys
from engine.ingress.download.paths import normalize_path, user_config_file
...
def test_ensure_era5_calls_cds_backends(tmp_path: Path) -> None:
    ...
    with patch("engine.ingress.download.backends.cds.download_era5_day", side_effect=fake_download):
...
def test_ensure_skips_existing_files(tmp_path: Path) -> None:
    ...
    with patch("engine.ingress.download.backends.cds.download_era5_day") as mocked:
        result = downloader.ensure(valid_time, cache_dir=cache)
