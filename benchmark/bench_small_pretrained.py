...
Examples::
    ...
    uv run python benchmark/bench_small_pretrained.py --data-dir /root/autodl-tmp/aurora
    uv run python benchmark/bench_small_pretrained.py --tiers fp32 fast_fp32 bf16_mixed --repeat 5
"""
...
_DEFAULT_TIERS: tuple[tuple[str, str], ...] = (
    ...
    ("fast_fp32", "Triton layout + PyTorch AdaLN/GELU"),
...
def main() -> None:
    ...
    parser.add_argument(
        "--compare-hf",
        action="store_true",
        help="Also print fp32 vs official test-output.pickle (absolute regression check)",
    )
    ...
    if args.compare_hf:
        ...
        if hf_ref is None:
            ...
            print("\n[compare-hf] skipped: no baseline forward captured")
        else:
            max_abs, mean_abs, max_rel, cos = _diff_vs_reference(hf_ref, baseline)
            print(
                f"\n[compare-hf] fp32 vs official test-output: "
